...
public class ChebiDTO implements Serializable {
    ...
    public void IdentifyLine(String line){
        ...
        if(line.regionMatches(true, 0, "id: CHEBI:", 0, 10)){
        ...
        if(line.regionMatches(true, 0, "synonym: ", 0, 9)){
            ...
            syn_ = new ChebiDTO_SYN(line.substring(10, line.lastIndexOf("[")-1).toLowerCase());
        ...
        if(line.regionMatches(true, 0, "is_a: CHEBI:", 0, 12)){
            //is_as.add(new ChebiDTO_ISA(line.substring(9, line.indexOf("!")-1)));
            is_as.add(new ChebiDTO_ISA(line.substring(12)));
        }
        
        //check for alt-id(s)
        if(line.regionMatches(true, 0, "alt_id: CHEBI:", 0, 14)){
            alt_ids.add(new ChebiDTO_ALT(line.substring(14)));
        }
        
        //check for xref(s)
        if(line.regionMatches(true, 0, "xref: ", 0, 6)){
            xrefs.add(new ChebiDTO_XREF(line.substring(6)));
        }
        ...
        if(line.regionMatches(true, 0, "relationship: ", 0, 14)){
            ...
            relationships.add(new ChebiDTO_REL(line.substring(14).trim()));
        }
        
        //check for subset(s)
        if(line.regionMatches(true, 0, "subset:", 0, 7)){
            subsets.add(new ChebiDTO_SUBSET(line.substring(7)));
        }
        
        //check for considers
        if(line.regionMatches(true, 0, "consider: CHEBI:", 0, 16)){
            considers.add(new ChebiDTO_CON(line.substring(16)));
        }
        
        //check for replaced_bys
        if(line.regionMatches(true, 0, "replaced_by: CHEBI:", 0, 19)){
            replaced_bys.add(new ChebiDTO_REP(line.substring(19)));
        }
