...
public class DoidDTO implements Serializable {
    ...
    public void IdentifyLine(String line){
        ...
        if(line.regionMatches(true, 0, "id: DOID:", 0, 9)){
        ...
        if(line.regionMatches(true, 0, "synonym: ", 0, 9)){
            ...
            syn_ = new DoidDTO_SYN(line.substring(10, line.lastIndexOf("[")-1).toLowerCase());
        ...
        if(line.regionMatches(true, 0, "is_a: DOID:", 0, 11)){
            is_as.add(new DoidDTO_ISA(line.substring(11, line.indexOf("!")-1)));
            //is_as.add(new DoidDTO_ISA(line.substring(11)));
        }
        
        //check for alt-id(s)
        if(line.regionMatches(true, 0, "alt_id: DOID:", 0, 13)){
            alt_ids.add(new DoidDTO_ALT(line.substring(13)));
        }
        
        //check for xref(s)
        if(line.regionMatches(true, 0, "xref: ", 0, 6)){
            xrefs.add(new DoidDTO_XREF(line.substring(6)));
        }
        ...
        if(line.regionMatches(true, 0, "relationship: ", 0, 14)){
            ...
            relationships.add(new DoidDTO_REL(line.substring(14).trim()));
        }
        
        //check for subset(s)
        if(line.regionMatches(true, 0, "subset:", 0, 7)){
            subsets.add(new DoidDTO_SUBSET(line.substring(7)));
        }
        
        //check for considers
        if(line.regionMatches(true, 0, "consider: DOID:", 0, 15)){
            considers.add(new DoidDTO_CON(line.substring(15)));
        }
        
        //check for replaced_bys
        if(line.regionMatches(true, 0, "replaced_by: DOID:", 0, 18)){
            replaced_bys.add(new DoidDTO_REP(line.substring(18)));
        }
