...
public class MaDTO implements Serializable {
    ...
    public void IdentifyLine(String line){
        ...
        if(line.regionMatches(true, 0, "id: MA:", 0, 7)){
        ...
        if(line.regionMatches(true, 0, "synonym: ", 0, 9)){
            synonyms.add(new MaDTO_SYN(line.substring(10, line.lastIndexOf("[")-1)));
        }
        
        //check for the mp terms' is_a(s)
        if(line.regionMatches(true, 0, "is_a: MA:", 0, 9)){
            is_as.add(new MaDTO_ISA(line.substring(9, line.indexOf("!")-1)));
        }
        
        //check for alt-id(s)
        if(line.regionMatches(true, 0, "alt_id: MA:", 0, 11)){
            alt_ids.add(new MaDTO_ALT(line.substring(11)));
        }
        
        //check for xref(s)
        if(line.regionMatches(true, 0, "xref: ", 0, 6)){
            xrefs.add(new MaDTO_XREF(line.substring(6)));
        }
        
        //check for relationship(s)
        if(line.regionMatches(true, 0, "relationship: ", 0, 14)){
            relationships.add(new MaDTO_REL(line.substring(14, line.indexOf("!")).trim()));
        }
