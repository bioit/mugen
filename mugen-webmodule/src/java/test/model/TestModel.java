...
//    }
    ...
    public void testStrain(PrintWriter out)
    {
        out.println("<pre>--- testStrain ---");
        ...
        try
        {
            ...
            StrainRemoteHome strainHome = (StrainRemoteHome)locator.getHome(ServiceLocator.Services.STRAIN);
            StrainTypeRemoteHome strainTypeHome = (StrainTypeRemoteHome)locator.getHome(ServiceLocator.Services.STRAIN_TYPE);
            StrainStateRemoteHome strainStateHome = (StrainStateRemoteHome)locator.getHome(ServiceLocator.Services.STRAIN_STATE);
            
            StrainRemote s1 = null;
            try
            {
                s1 = strainHome.findByPrimaryKey(new Integer(99));
            }
            catch (ObjectNotFoundException e)
            {
                s1 = strainHome.create(99,"test",getCaller());
            }
            ...
            out.print("Designation="+s1.getDesignation());
            
            
            s1.setDesignation("test2");
            out.print("Designation="+s1.getDesignation());
            StrainTypeRemote st1 = null;
            try
            {
                st1 = strainTypeHome.findByPrimaryKey(new Integer(99));    
            } 
            catch (Exception ex)
            {
                st1 = strainTypeHome.create(99,"test","tst", getCaller());
            }
            ...
            try
            {
                state = strainStateHome.findByPrimaryKey(new Integer(99));
            } 
            catch (Exception ex)
            {
                state = strainStateHome.create(99, "state", "st", getCaller());
            }
            try
            {
            ...
            state.setAbbreviation("st22");
            
            
            model.setStrain(s1);
            
            model.setStrain(null);
            try
            {
                
                s1.removeState(state);
                s1.removeType(st1);
